...
void NETWORK_Close(client_t *p_client)
{
    ...
    /* Dua tren network_id mà p_client dang so huu, dat gia tri client va server
     * trong netwwork ve NULL*/
...
void NETWORK_ExchangeData(int network_id, char *command)
{
    ...
    {
        SERVER_HandleRequest_ModuleCalculation(
            network[network_id].p_server,
            network[network_id].p_client->network_stream, response);
    }
    else
    {
        SERVER_HandleRequest_StandardCalculation(
            network[network_id].p_server,
            network[network_id].p_client->network_stream, response);
    }
